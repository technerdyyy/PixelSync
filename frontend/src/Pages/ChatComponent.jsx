      ...
      <section className="h-[100vh] w-full flex flex-col">
        ...
        <div className="flex-1 p-4 overflow-y-auto bg-gray-100">
          {messages.map((message, index) => {
            const isSent = message.senderId === socket.id;

            return (
              <div
                key={index}
                className={`flex items-start gap-2 mb-4 max-w-[30%] w-[30%] p-3 rounded-lg 
                                    ${
                                      isSent
                                        ? "ml-auto flex-row-reverse bg-[#6F0081] text-white"
                                        : "mr-auto bg-gray-200 text-black"
                                    }`}
              >
                <img
                  src={message.img || user}
                  alt="User"
                ...
                <div>
                  <p
                    className={`text-xs font-bold mb-1 ${
                      isSent ? "text-purple-200" : "text-gray-700"
                    }`}
                  >
        ...
        <div className="p-4 border-t border-gray-200 bg-white">
          ...
          >
            ...
            <button
              type="submit"
              className="p-3 bg-[#6F0081] text-white rounded-lg hover:bg-purple-700"
            >
              <svg
